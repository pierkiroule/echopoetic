...
- `src/App.jsx` : landing minimaliste et poétique.
- `src/components/Sky.jsx` : arrière-plan animé par CSS.
...
- `src/styles/poetry.css` : style global et animations légères.
...
## Notes techniques
- Solo-only : aucune dépendance réseau ou P2P. Les données vivent dans `localStorage` et peuvent être vidées via l'importeur.
- Minimalisme : pas de dépendance supplémentaire ; animations via CSS et composants React sobres.
- Déploiement : Vite + React, configuration Vercel fournie (`npm run build`).
